...
class Module {
    ...
    public function onRoute(\Zend\EventManager\EventInterface $e) { // Event manager of the app
        ...
        if ($auth->hasIdentity()) {
            ...
            $role = 'member'; //privremeno zakucano
            
            /* podaci za layout */
            $imePrezime = $user->getIme() . ' ' . $user->getPrezime();
            $viewModel = $application->getMvcEvent()->getViewModel();
            $viewModel->korisnikImePrezime= $imePrezime;
        }
        ...
        if (!$acl->hasResource($controller)) {
            throw new \Exception('Resource ' . $controller . '/' . $action . ' not defined');
        }
